...
namespace GradesPrototype.Data
{
    ...
    public abstract class User
    {
        ...
        public string Password
        {
            set
            {
                ...
                if (!SetPassword(value)) // SetPassword returns a bool value whether the password completed the verification, if true is returned (!SetPassword) SetPassword is called and the value is the string provided within the method call.
                {
                    throw new ArgumentException("Did not set password, needs more complexity.");
                }
                else
                {
                    _password = value;
    ...
    public class Student: User, IComparable<Student>
    {
        ...
        //Implementing the abstract method SetPassword() in the base class:
    ...
    public class Teacher : User
    {
        ...
        public override bool SetPassword(string password)
        {
            Match matchTeacherPass = Regex.Match(password, @".*[0-9]+.*[0-9]+.*");
            if (matchTeacherPass.Success && password.Length >= 8)
            {
                _password = password;
                return true;
            }
            return false; //this does not HAVE to be included in an else statement apparently.
        }
